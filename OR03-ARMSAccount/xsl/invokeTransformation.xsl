         ...
         <ns0:body>
            <xsl:for-each select="/ns0:AccountToARMS/ns0:body/ns0:e-CLEARSAccount">
               <ns0:e-CLEARSAccount>
                  ...
                  <xsl:variable name="AccType" select="oraext:query-database(concat(&quot;select target from transformation where service_name='OR03' and source='&quot;,ns0:AccountType,&quot;'&quot;),false(),false(),&quot;jdbc/ds_kpeimid&quot;)"/>
                  <xsl:choose>
                     <xsl:when test="string-length(oraext:right-trim(oraext:left-trim($AccType))) > 0">
                        <ns0:AccountType>
